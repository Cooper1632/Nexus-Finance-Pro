                                ...
                                <tr key={d.id}>
                                    <td className="no-print" style={{ textAlign: 'center' }}>
                                        <input
                                            id={`check-${d.id}`}
                                            name={`check-${d.id}`}
                                            aria-label="Activer/Désactiver le suivi"
                                            type="checkbox"
                                            checked={d.checked}
                                    ...
                                    <td><CurrencyInput id={`init-${d.id}`} name={`init-${d.id}`} aria-label="Montant initial" value={d.montantInitial} onChange={(e) => updateDebtField(d.id, 'montantInitial', e.target.value)} placeholder="0.00" className="currency-input" style={{ backgroundColor: '#F8F9FA' }} /></td>
                                    <td><CurrencyInput id={`montant-${d.id}`} name={`montant-${d.id}`} aria-label="Solde actuel" value={d.montant} onChange={(e) => updateDebtField(d.id, 'montant', e.target.value)} placeholder="0.00" className="currency-input" /></td>
                                    <td><input id={`taux-${d.id}`} name={`taux-${d.id}`} aria-label="Taux d'intérêt" type="number" value={d.taux ? parseFloat((d.taux * 100).toFixed(2)) : ''} onChange={(e) => updateDebtField(d.id, 'taux', e.target.value)} placeholder="0" /></td>
                                    <td><CurrencyInput id={`min-${d.id}`} name={`min-${d.id}`} aria-label="Paiement minimum" value={d.paiementMin} onChange={(e) => updateDebtField(d.id, 'paiementMin', e.target.value)} placeholder="Auto" className="currency-input" style={{ borderColor: 'var(--warning-color)' }} /></td>
                        ...
                        <div className="plan-switch-container" style={{ margin: '0', transform: 'scale(1.1)', padding: '8px 20px', display: 'flex', alignItems: 'center' }}>
                            ...
                            <label className="toggle-switch" htmlFor="strategy-switch" style={{ width: '60px', height: '30px' }}>
                                <input
                                    ...
                                    aria-label="Choisir la stratégie"
                                    type="checkbox"
                    ...
                    <div className="plan-explanation-box">
                        <p style={{ margin: 0, fontSize: '1rem', lineHeight: '1.6' }}>
                            {(() => {
                                ...
                                return (
                                    <>
                                        Avec un budget mensuel de <strong>{formatCurrency(realBudget)}</strong>
                                        {isDeficit ? (
                                            <span style={{ color: '#E74C3C', marginLeft: '5px', fontWeight: 'bold' }}>
                                                (insuffisant pour couvrir les {formatCurrency(minRequired)} requis)
                                            </span>
                                        ) : (
                                            <span> (couvrant les {formatCurrency(minRequired)} requis)</span>
                                        )}
                                        , vous {isDeficit ? 'avez un déficit mensuel de' : 'disposez de'} <strong style={{ color: isDeficit ? '#E74C3C' : 'var(--success-color)' }}>{formatCurrency(Math.abs(diff))}</strong> {isDeficit ? 'à combler.' : 'supplémentaires chaque mois pour accélérer le remboursement de votre dette cible.'}
                                    </>
                                );
                            })()}
                        </p>
